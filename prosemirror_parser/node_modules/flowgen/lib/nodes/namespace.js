...
class Namespace extends _node.default {
  constructor(_name, _functions, property) {
    super(null);
    ...
    _defineProperty(this, "print", (namespace = "", mod = "root", depth) => {
      const children = (0, _lodash.uniqBy)((0, _lodash.orderBy)(this.getChildren(), [a => a.isValue], ["desc"]), child => child.name.text || child.name);
      ...
      const enums = children.filter(child => child.raw && child.raw.kind === ts.SyntaxKind.EnumDeclaration);
      ...
      if (this.functions.length || functions.length || variables.length || namespaces.length || classes.length || enums.length) {
        let topLevel = "";
        const nsGroup = `
      declare var npm$namespace$${name}: {|
        ${this.functions.map(propNode => `${_printers.default.functions.functionType(propNode.raw, true)},`).join("\n")}
        ${functions.map(child => {
