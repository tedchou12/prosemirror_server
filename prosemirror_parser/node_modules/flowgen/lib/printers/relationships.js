...
const moduleExports = node => {
  let name = _index.default.node.printType(node.expression);
...
const importExportSpecifier = node => {
  if (node.propertyName) {
    return `${_index.default.node.printType(node.propertyName)} as ${_index.default.node.printType(node.name)}`;
  }

  return _index.default.node.printType(node.name);
